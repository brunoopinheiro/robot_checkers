from math import pi, sin


Normalizacao = 552
# (piece) [x2, y1, x1, y2]
a1 = [552, 30, 502, 81]
a3 = [0]*4; a5 = [0]*4; a7 = [0]*4
b2 = [0]*4; b4 = [0]*4; b6 = [0]*4; b8 = [0]*4
c1 = [0]*4; c3 = [0]*4; c5 = [0]*4; c7 = [0]*4
d2 = [0]*4; d4 = [0]*4; d6 = [0]*4; d8 = [0]*4
e1 = [0]*4; e3 = [0]*4; e5 = [0]*4; e7 = [0]*4
f2 = [0]*4; f4 = [0]*4; f6 = [0]*4; f8 = [0]*4
g1 = [0]*4; g3 = [0]*4; g5 = [0]*4; g7 = [0]*4
h2 = [0]*4; h4 = [0]*4; h6 = [0]*4; h8 = [0]*4


Para executar ações baseadas no nome das variáveis dentro de uma lista, você pode armazenar as variáveis em um dicionário, onde as chaves são os nomes das variáveis e os valores são as próprias variáveis. Dessa forma, você pode verificar os nomes das variáveis e agir de acordo com eles. Aqui está um exemplo de como fazer isso:

python
Copy code
# Definindo as listas de exemplo
a1, a3, a5, a7 = [1], [2], [3], [4]
b2, b4, b6, b8 = [5], [6], [7], [8]
c1, c3, c5, c7 = [9], [10], [11], [12]
d2, d4, d6, d8 = [13], [14], [15], [16]
e1, e3, e5, e7 = [17], [18], [19], [20]
f2, f4, f6, f8 = [21], [22], [23], [24]
g1, g3, g5, g7 = [25], [26], [27], [28]
h2, h4, h6, h8 = [29], [30], [31], [32]

# Criando um dicionário com os nomes e as variáveis
variaveis = {
    'a1': a1, 'a3': a3, 'a5': a5, 'a7': a7,
    'b2': b2, 'b4': b4, 'b6': b6, 'b8': b8,
    'c1': c1, 'c3': c3, 'c5': c5, 'c7': c7,
    'd2': d2, 'd4': d4, 'd6': d6, 'd8': d8,
    'e1': e1, 'e3': e3, 'e5': e5, 'e7': e7,
    'f2': f2, 'f4': f4, 'f6': f6, 'f8': f8,
    'g1': g1, 'g3': g3, 'g5': g5, 'g7': g7,
    'h2': h2, 'h4': h4, 'h6': h6, 'h8': h8
}

largura_horizontal = 553-153
largura_vertical = 430-30
degree_vertical = 90-88.69
degree_horizonal = 180-178.16

cateto_oposto_y = sin((pi/180)*degree_horizonal)*largura_horizontal

cateto_oposto_x = sin((pi/180)*degree_vertical)*largura_vertical

def coordenadas(casa_a1, lista_casas):
    for casa in lista_casas:
        a = 1/16
        b = 0
        for i in range(len(casa)):
            if i%2 != 0: #y1 and y2
                casa[i] = casa_a1[i]*(cateto_oposto_y*((1-a)))
                a += 1/4
            else:
                casa[i] = casa_a1[i]*(cateto_oposto_x*((1+b)))
                b += 1/8
    for i in lista_casas:
        soma = sum(i)
        for indice in range(4):
            i[indice] = i[indice]/soma
    return lista_casas
                    
mapeadas = coordenadas(a1, minha_lista)
print(mapeadas)

lista_casas = ['a1', 'a3', 'a5', 'a7', \
               'b2', 'b4', 'b6', 'b8', \
                'c1', 'c3', 'c5', 'c7', \
                'd2', 'd4', 'd6', 'd8', \
                'e1', 'e3', 'e5', 'e7', \
                'f2', 'f4', 'f6', 'f8', \
                'g1', 'g3', 'g5', 'g7', \
                'h2', 'h4', 'h6', 'h8', \
    ]
n_digitos = 14
with open(f'docs/checkers_mapping.txt', 'w') as f:
    conteudo = 'Checkers mapped'
    for indice, casa in zip(range(32), lista_casas):
        conteudo += f'\n{casa}'
        for coordenada in mapeadas[indice]:
            conteudo += '  ' +str(round(coordenada, n_digitos))
    f.write(conteudo)

        

