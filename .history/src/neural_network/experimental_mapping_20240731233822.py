...
def mapping_bounding_box(dict_yolo, dict_checkers, tolerancia=10):
    locations = []
    ...
    for piece in dict_yolo:
        ...
        for square, coords in dict_checkers.items():
            ...
            if (
                (x1_square - tolerancia <= box_piece['x1'] <= x2_square + tolerancia) and
                (y1_square - tolerancia <= box_piece['y1'] <= y2_square + tolerancia) and
                (x1_square - tolerancia <= box_piece['x2'] <= x2_square + tolerancia) and
                (y1_square - tolerancia <= box_piece['y2'] <= y2_square + tolerancia)
            ):
...
robot_board = int(input('kAnova (2) or kinova (1)?: q to exit'))
while robot_board not in [1, 2]:
        robot_board = int(input('kAnova (2) or kinova (1)?: '))
...
res = mapping_bounding_box(dict_yolo_example, dicionario_kAnova)
