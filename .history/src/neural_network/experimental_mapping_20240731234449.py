...
def mapping_bounding_box(dict_yolo, dict_checkers, tolerance=10):
    location = []
    for piece in dict_yolo:
        ...
        for square, coords in dict_checkers.items():
            ...
            if (
                (x1_square - tolerance <= box_piece['x1'] <= x2_square + tolerance) and
                (y1_square - tolerance <= box_piece['y1'] <= y2_square + tolerance) and
                (x1_square - tolerance <= box_piece['x2'] <= x2_square + tolerance) and
                (y1_square - tolerance <= box_piece['y2'] <= y2_square + tolerance)
            ):
                location.append((piece, square))
                found = True
                break
        if not found:
            location.append((piece, None))
    return location
