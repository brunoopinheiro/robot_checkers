...
def mapping_bounding_box(dict_yolo, dict_checkers_kanova, tolerancia=10):
    ...
    for piece in dict_yolo:
        ...
        for casa, coords in dict_checkers_kanova.items():
            x1_house, y1_house, x2_casa, y2_casa = coords

            if (
                (x1_house - tolerancia <= box_piece['x1'] <= x2_casa + tolerancia) and
                (y1_house - tolerancia <= box_piece['y1'] <= y2_casa + tolerancia) and
                (x1_house - tolerancia <= box_piece['x2'] <= x2_casa + tolerancia) and
                (y1_house - tolerancia <= box_piece['y2'] <= y2_casa + tolerancia)
            ):
                locations.append((piece, casa))
                found = True
                break
...
for piece, casa in resultados:
    print(f"A peça {piece['name']} está localizada na casa {casa}")
