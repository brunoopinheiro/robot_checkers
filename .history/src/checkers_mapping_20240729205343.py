...
def coordenadas(casa_a1, dicionario_casas):
    for id, casa in dicionario_casas.items():
        ...
        b = 0
        for i in range(4):
            if i % 2 != 0 and any(char in id for char in ['a', 'c', 'e', 'g']):  # y1 and y2
                casa[id][i] = casa_a1[id][i]*(cateto_oposto_y*((1-a)))
                a += 1/4
       
            if i%2 == 0 and any(char in id for char in ['a', 'c', 'e', 'g']): # x1 and x2
                casa[id][i] = casa_a1[id][i]*(cateto_oposto_x*((1+b)))+shift_x
                b += 1/8
                shift_x += 50
